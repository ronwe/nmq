...
exports.startClient = function(config){
	var _cs = []
		...
		,cmd = {
			...
			,'onsub': function(drawer , msg){
				if (!_cbkOnSub[drawer] || !_cbkOnSub[drawer].length) return
				for (var i = 0 , j = _cbkOnSub[drawer].length; i < j ; i++){
	...
	return {
		...
		,unSub : function(drawer , subId){
			...
			if (subId >= 0 ) _cbkOnSub[drawer].splice(subId , 1)
			else delete _cbkOnSub[drawer]
		}
		,sub : function(drawer , cbk , opt){
			if (! drawer || 'function' != typeof cbk) return false
			if (opt && opt.cover) _cbkOnSub[drawer] = []
			else if (! (drawer in  _cbkOnSub)) _cbkOnSub[drawer] = []
			return _cbkOnSub[drawer].push(cbk) - 1
		}
		,pull : pull 
		,clean : function(drawer ){
